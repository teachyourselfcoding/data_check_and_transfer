...
from modules_evaluation import generate_evaluation_result,prediction_evaluation_iteration
...
class DataCollectionUpload():
    ...
    def __init__(self, path):
        ...
        self.check_file_name_list = loadTag(tag_file_name='data_check.json')
        self.tag_module_list = loadTag(tag_file_name='tag_module.json')
        self.post = True
        ...
        self.tprofile_thresh = loadTag('tprofile_thresh.json', '')
        self.readShellFile('download_logs.sh')
    ...
    def matchBackUpTag(self,dir_path):
        try:
            ...
            for backup_tag in self.backup_tag_list:
                ...
                print(tag_gap)
                if abs(tag_gap) < 4:
                    tag_data = loadTag(self.input_data_path,backup_tag_name)
    ...
    def generateCase(self,dir_path):
        case_finder = "~/ws/master_case_finder/senseauto/build/modules/simulator/tools/scenario_log_tools/case_finder "
        input_log = dir_path+'simulator_scenario/simulator_scenario_log.bin '
        output_path = dir_path+"finder_case.json"
        os.system(case_finder+input_log+output_path)
    ...
    def segmentPreprationCollection(self, dir_name):
        ...
        finder_path = self.input_data_path+dir_name+'/finder_case.json'
        if os.path.exists(finder_path):
            finder_case_json = loadTag(finder_path,'')
        if finder_case_json == []:
            return
        self.dataSegment(dir_name, finder_case_json)
    ...
    def dataSegment(self, dir_name, segment_point):
        ...
        for finder_tag in segment_point:
            ...
            record_tag["start_format"] =  self.unixToBjclock(finder_tag["case_timestamp"]).split(" ",-1)[1].replace('_',':')
    ...
    def unixToBjclock(self,unix_time):
        ...
        now_time_style = time.strftime("%Y_%m_%d %H_%M_%S", time_array)
    ...
    def prePredictionEval(self,dir_path,prediction_tagging_list):
        ...
        eval_bad_result = loadTag(dir_path,'prediction_evaluation/result/bad_cases.json')
        input_list = []
    ...
    def falseDataUpload(self, dir_path_without):
        false_data_tag = loadTag(dir_path_without)
    ...
    def sliceDataCheck(self, dir_path):
        ...
        for slice_data_tag_path in data_tag_paths:
            ...
            slice_data_tag = loadTag(slice_data_tag_path, tag_file_name='')
    ...
    def getDataCollectionDstLink(self, tag_info, data_month,slice):
        ...
        if tag_info["topic_name"][0] == "repo_master":
            if slice:
                return "s3://sh40_fieldtest_dataset/"+data_month+"/master/segment_data/"
            else:
                return "s3://sh40_fieldtest_dataset/"+data_month+"/master/raw_data/"
        else:
            if slice:
                return "s3://sh40_fieldtest_dataset/"+data_month+"/feature/segment_data/"
            else:
                return "s3://sh40_fieldtest_dataset/"+data_month+"/feature/raw_data/"
